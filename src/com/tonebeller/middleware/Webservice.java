...
//			this.sendPost(req, resultText.toString());
  ...
  private String GetPageContentPOST(JSONObject req) throws Exception {
	  System.out.println("\n\n ====== GET PAGE CONTENT POST Request === \n\n");
		URL obj = new URL(req.getString("url"));
		HttpURLConnection conn = (HttpURLConnection) obj.openConnection();
		conn.setDoOutput(true);
		// default is GET
		...
		DataOutputStream wr = new DataOutputStream(conn.getOutputStream());
		wr.writeBytes(req.getString("GET"));
		wr.flush();
		wr.close();
		
		System.out.println("Cookie: " + this.cookies);
		int responseCode = conn.getResponseCode();
		System.out.println("\nGET CONTENT: Sending 'POST' request to URL : " + req.getString("url"));
		System.out.println("Response Code : " + responseCode);
		  ...
		  StringBuilder resultText = new StringBuilder();
			...
			while( keys.hasNext() ) {
				...
				if (resultText.length() == 0) {
					resultText.append(key + "=" + result.getString(key));
				} else {
					resultText.append("&" + key + "=" + result.getString(key));
				}
			}
